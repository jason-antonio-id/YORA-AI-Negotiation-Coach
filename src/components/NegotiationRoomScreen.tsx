...
export function NegotiationRoomScreen({ onNavigate, supplier, onUpdateSupplier, userData }: NegotiationProps) {
  ...
  const handleSavePrice = async () => {
    ...
    try {
      ...
      // Reflect the change locally right away instead of waiting for a refetch -
      // this is also what the PDF export reads from, so this keeps it in sync.
      onUpdateSupplier({ ...supplier, currentPrice: tempPrice });
      setIsEditingPrice(false);
    } catch (error) {
      console.error("Failed to update current price:", error);
      alert("Failed to save the current price. Please try again.");
    }
  };
  ...
  const handleExportHistory = async () => {
    ...
    try {
      ...
      // Draw supplier icon on the left. We draw the red circle background ourselves
      // (reliable, always renders) and place the white icon image on top of it -
      // rather than trusting the source PNG to already contain a red background.
      const cxSummaryIcon = 24.5;
      const cySummaryIcon = yPos + 10.5;
      const rSummaryIcon = 4.5;
      doc.setFillColor(183, 28, 28);
      doc.circle(cxSummaryIcon, cySummaryIcon, rSummaryIcon, 'F');
      ...
      if (isSummaryIconValid) {
        try {
          // Slightly inset the icon image within the red circle so it doesn't touch the edges,
          // and preserve its real aspect ratio instead of stretching it into a square.
          const insetDiameter = rSummaryIcon * 1.6;
          const aspect = summarySupplierIconData.aspect || 1.0;
          const w = aspect >= 1 ? insetDiameter : insetDiameter * aspect;
          const h = aspect >= 1 ? insetDiameter / aspect : insetDiameter;
          doc.addImage(summarySupplierIconData.dataUrl, 'PNG', cxSummaryIcon - w / 2, cySummaryIcon - h / 2, w, h);
        } catch (e) {
          drawBusinessIcon(cxSummaryIcon, cySummaryIcon, rSummaryIcon, true);
        }
      } else {
        drawBusinessIcon(cxSummaryIcon, cySummaryIcon, rSummaryIcon, true);
      }
      ...
      doc.setFontSize(7.5);
      doc.setTextColor(30, 30, 30);
      doc.text(`CNY ${cleanStringForPDF(supplier.currentPrice || supplier.walkAwayPrice || 'N/A')}`, 32.0, yPos + 31);
      ...
      const drawTimelineNode = (cx: number, cy: number, type: 'ai' | 'supplier' | 'star') => {
        ...
        // Stop the line short of both circles (radius + a small gap) instead of drawing
        // center-to-center, so it never visually overlaps either avatar's icon no matter
        // what order things get painted in.
        const avatarRadius = 2.8;
        const lineGap = 1.0;
        if (lastAvatarY !== null && currentPage === lastAvatarPage) {
          const lineStart = lastAvatarY + avatarRadius + lineGap;
          const lineEnd = cy - avatarRadius - lineGap;
          if (lineEnd > lineStart) {
            doc.setDrawColor(183, 28, 28);
            doc.setLineWidth(0.35); // 0.35mm looks beautiful and crisp
            doc.line(cx, lineStart, cx, lineEnd);
          }
        }
        ...
        if (type === 'ai') {
          // 'ai' here means the buyer/user's own messages (see nodeType mapping above) - not Rui.
          ...
          if (isValidImg) {
            try {
              const diameter = 2.8 * 2;
              const aspect = transcriptSupplierIconData.aspect || 1.0;
              const w = aspect >= 1 ? diameter : diameter * aspect;
              const h = aspect >= 1 ? diameter / aspect : diameter;
              doc.addImage(transcriptSupplierIconData.dataUrl, 'PNG', cx - w / 2, cy - h / 2, w, h);
            } catch (e) {
          ...
          // 'star' is what actually renders Rui's own messages (see nodeType mapping above).
          // White backing circle masks the connecting line behind the icon.
          doc.setFillColor(255, 255, 255);
          doc.circle(cx, cy, 2.8, 'F');
          const isValidImg = topRightMascotData.dataUrl && topRightMascotData.dataUrl.startsWith('data:image');
          if (isValidImg) {
            try {
              // Fit the image within the circle's diameter without stretching/squishing it -
              // scale to the actual aspect ratio, capped to the diameter on the longer side.
              const diameter = 2.8 * 2;
              const aspect = topRightMascotData.aspect || 1.0;
              const w = aspect >= 1 ? diameter : diameter * aspect;
              const h = aspect >= 1 ? diameter / aspect : diameter;
              doc.addImage(topRightMascotData.dataUrl, 'PNG', cx - w / 2, cy - h / 2, w, h);
            } catch (e) {
              doc.setDrawColor(183, 28, 28);
              doc.setLineWidth(0.25);
